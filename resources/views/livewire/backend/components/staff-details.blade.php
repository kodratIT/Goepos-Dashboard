                ...
                <table id="documentsTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                                    ...
                                    <tr
                                        ...
                                        <td class="px-6 whitespace-nowrap">
                                            @if (isset($data->qris))
                                                {{-- @if ($data->qris->enabledByGoePos) --}}
                                                <a id="disabledButtom"
                                                    class="{{ $data->qris->enabledByGoePos ? '' : 'hidden' }} font-medium text-red-600 dark:text-red-500 hover:underline"
                                                    style="margin-right: 20px;">Disabled</a>
                                                <a id="disabledButtonloading" class="hidden " style="margin-right: 20px;"
                                                    wire:click.prevent="qrisDisabled" wire:loading.attr="disabled"
                                                    wire:loading.class="" wire:target="qrisDisabled">
                                                    <!-- Ikon pemuatan (spinner) yang muncul saat proses pemuatan -->
                                                    <svg wire:loading wire:target="qrisDisabled" aria-hidden="true"
                                                        role="status" class="inline w-4 h-4 me-3 text-black animate-spin"
                                                ...
                                                {{-- @else --}}
                                                <a id="activeButtom"
                                                    class="{{ !$data->qris->enabledByGoePos ? '' : 'hidden' }} font-medium text-blue-600 dark:text-red-500 hover:underline"
                                                ...
                                                @else
                                                    ...
                                                    <button type="button" id="confirm-activate"
                                                        ...
                                                        <span wire:loading.remove
                                                            wire:target="activateQris">Activate</span>
                        ...
                        @else
                            <tr>
                                <td colspan="3"
                                    class="px-6 whitespace-nowrap text-center text-gray-500 dark:text-gray-400">Data tidak
                                    ditemukan</td>
                            </tr>
                        @endif
                    </tbody>
    ...
    <script>
        document.addEventListener('livewire:init', function() {
            Livewire.on('documentsLoaded', () => {
                console.log('documentsLoaded event triggered');
                initializeSweetAlert();
            });

            Livewire.on('qrisActivated', () => {
                console.log('qrisActivated event triggered');
                Swal.fire({
                    title: 'Berhasil!',
                    text: 'Metode Pembayaran QRIS Telah Diaktifkan',
                    icon: 'success',
                    confirmButtonText: 'OK'
                }).then(() => {
                    // Trigger Livewire component re-render without full page reload
                    Livewire.emit('refreshComponent');
                });
            });

            Livewire.on('qrisDisabled', () => {
                console.log('qrisDisabled event triggered');
                Swal.fire({
                    title: 'Berhasil!',
                    text: 'Metode Pembayaran QRIS Telah Dinonaktifkan',
                    icon: 'success',
                    confirmButtonText: 'OK'
                }).then(() => {
                    // Trigger Livewire component re-render without full page reload
